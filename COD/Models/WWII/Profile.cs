namespace COD.Models.WWII
{
	public class Profile
	{
		public class All
		{
			public double kills { get; set; }
			public double accuracy { get; set; }
			public double losses { get; set; }
			public double timePlayedAllies { get; set; }
			public double points { get; set; }
			public double score { get; set; }
			public double totalXp { get; set; }
			public double headshots { get; set; }
			public double plants { get; set; }
			public double deaths { get; set; }
			public double wins { get; set; }
			public double killStreak { get; set; }
			public double kdRatio { get; set; }
			public double destructions { get; set; }
			public double captures { get; set; }
			public double bestScore { get; set; }
			public double winStreak { get; set; }
			public double bestKills { get; set; }
			public double timePlayed { get; set; }
			public double matchesPlayed { get; set; }
			public double suicides { get; set; }
			public double money { get; set; }
			public double currentWinStreak { get; set; }
			public double prestigeShopTokens { get; set; }
			public double matchesCompleted { get; set; }
			public double unlockPoints { get; set; }
		}

		public class Blades
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double throwingKnifeKills { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostThrowingKnifeKills { get; set; }
			public double ties { get; set; }
			public double mostMeleeKnifeKills { get; set; }
			public double meleeKnifeKills { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Gun
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double stabs { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostStabs { get; set; }
			public double ties { get; set; }
			public double mostGunPromotions { get; set; }
			public double timeStampFirstGame { get; set; }
			public double gunPromotions { get; set; }
		}

		public class Twar
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostKillWhileCaptures { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostCaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double killWhileCaptures { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Dom
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostdefends { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostcaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double defends { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class DmHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double headshots { get; set; }
			public double mostkills { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double bestkdr { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double assists { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class ConfHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double denies { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostconfirms { get; set; }
			public double ties { get; set; }
			public double confirms { get; set; }
			public double mostdenies { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Hp
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostdefends { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostcaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double defends { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double totalHPTime { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class TwarHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostKillWhileCaptures { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostCaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double killWhileCaptures { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Dm
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double headshots { get; set; }
			public double mostkills { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double bestkdr { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double assists { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Conf
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double denies { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostconfirms { get; set; }
			public double ties { get; set; }
			public double confirms { get; set; }
			public double mostdenies { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Demo
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double plants { get; set; }
			public double detonates { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostdefuses { get; set; }
			public double ties { get; set; }
			public double defuses { get; set; }
			public double mostplants { get; set; }
			public double timeStampFirstGame { get; set; }
			public double mostdetonates { get; set; }
		}

		public class Ball
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double killedBallCarrier { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double mostKilledBallCarrier { get; set; }
			public double mostPointsScored { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double ties { get; set; }
			public double pointsScored { get; set; }
			public double carries { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Sd
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double plants { get; set; }
			public double detonates { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostdefuses { get; set; }
			public double ties { get; set; }
			public double defuses { get; set; }
			public double mostplants { get; set; }
			public double timeStampFirstGame { get; set; }
			public double mostdetonates { get; set; }
		}

		public class SrHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double denies { get; set; }
			public double mostconfirms { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double mostdenies { get; set; }
			public double plants { get; set; }
			public double detonates { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostdefuses { get; set; }
			public double ties { get; set; }
			public double defuses { get; set; }
			public double confirms { get; set; }
			public double mostplants { get; set; }
			public double timeStampFirstGame { get; set; }
			public double mostdetonates { get; set; }
		}

		public class RaidHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostDefends { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double destructs { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double constructs { get; set; }
			public double defends { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostDestructs { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
			public double mostConstructs { get; set; }
		}

		public class Ctf
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostreturns { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostcaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double returns { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Onevone
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostkills { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double bestkdr { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Sr
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double denies { get; set; }
			public double mostconfirms { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double mostdenies { get; set; }
			public double plants { get; set; }
			public double detonates { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostdefuses { get; set; }
			public double ties { get; set; }
			public double defuses { get; set; }
			public double confirms { get; set; }
			public double mostplants { get; set; }
			public double timeStampFirstGame { get; set; }
			public double mostdetonates { get; set; }
		}

		public class CtfHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostreturns { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostcaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double returns { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class War
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double headshots { get; set; }
			public double mostkills { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double bestkdr { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double assists { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Control
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostdefends { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostcaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double defends { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class SdHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double plants { get; set; }
			public double detonates { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostdefuses { get; set; }
			public double ties { get; set; }
			public double defuses { get; set; }
			public double mostplants { get; set; }
			public double timeStampFirstGame { get; set; }
			public double mostdetonates { get; set; }
		}

		public class DomHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostdefends { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostcaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double defends { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class WarHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double headshots { get; set; }
			public double mostkills { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double bestkdr { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double assists { get; set; }
			public double ties { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class InfectHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostSurvivorKills { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double infectedKills { get; set; }
			public double losses { get; set; }
			public double survivorKills { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double ties { get; set; }
			public double mostInfectedKills { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class BallHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double killedBallCarrier { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double mostKilledBallCarrier { get; set; }
			public double mostPointsScored { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double ties { get; set; }
			public double pointsScored { get; set; }
			public double carries { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class GunHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double stabs { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double mostStabs { get; set; }
			public double ties { get; set; }
			public double mostGunPromotions { get; set; }
			public double timeStampFirstGame { get; set; }
			public double gunPromotions { get; set; }
		}

		public class HpHc
		{
			public double timePlayed { get; set; }
			public double xpEarned { get; set; }
			public double mostdefends { get; set; }
			public double kills { get; set; }
			public double timeStampLastGame { get; set; }
			public double mostcaptures { get; set; }
			public double wins { get; set; }
			public double losses { get; set; }
			public double defends { get; set; }
			public double score { get; set; }
			public double deaths { get; set; }
			public double numMatches { get; set; }
			public double captures { get; set; }
			public double ties { get; set; }
			public double totalHPTime { get; set; }
			public double timeStampFirstGame { get; set; }
		}

		public class Mode
		{
			public Blades blades { get; set; }
			public Gun gun { get; set; }
			public Twar twar { get; set; }
			public Dom dom { get; set; }
			public DmHc dm_hc { get; set; }
			public ConfHc conf_hc { get; set; }
			public Hp hp { get; set; }
			public TwarHc twar_hc { get; set; }
			public Dm dm { get; set; }
			public Conf conf { get; set; }
			public Demo demo { get; set; }
			public Ball ball { get; set; }
			public Sd sd { get; set; }
			public SrHc sr_hc { get; set; }
			public RaidHc raid_hc { get; set; }
			public Ctf ctf { get; set; }
			public Onevone onevone { get; set; }
			public Sr sr { get; set; }
			public CtfHc ctf_hc { get; set; }
			public War war { get; set; }
			public Control control { get; set; }
			public SdHc sd_hc { get; set; }
			public DomHc dom_hc { get; set; }
			public WarHc war_hc { get; set; }
			public InfectHc infect_hc { get; set; }
			public BallHc ball_hc { get; set; }
			public GunHc gun_hc { get; set; }
			public HpHc hp_hc { get; set; }
		}

		public class Lifetime
		{
			public All all { get; set; }
			public Mode mode { get; set; }
		}

		public class All2
		{
			public double kills { get; set; }
			public double shotsMissed { get; set; }
			public double kdRatio { get; set; }
			public double distanceTravelled { get; set; }
			public double divisionXpMountain { get; set; }
			public double accuracy { get; set; }
			public double divisionXpExpeditionary { get; set; }
			public double divisionXpInfantry { get; set; }
			public double divisionXpArmored { get; set; }
			public double losses { get; set; }
			public double shotsLanded { get; set; }
			public double divisionXpAirborne { get; set; }
			public double avgSpeed { get; set; }
			public double avgKillDistance { get; set; }
			public double score { get; set; }
			public double totalXp { get; set; }
			public double timePlayed { get; set; }
			public double headshots { get; set; }
			public double matchesPlayed { get; set; }
			public double divisionXpNone { get; set; }
			public double assists { get; set; }
			public double scorePerMinute { get; set; }
			public double shotsFired { get; set; }
			public double deaths { get; set; }
		}

		public class Raid
		{
			public double kills { get; set; }
			public double shotsMissed { get; set; }
			public double kdRatio { get; set; }
			public double distanceTravelled { get; set; }
			public double divisionXpMountain { get; set; }
			public double accuracy { get; set; }
			public double divisionXpExpeditionary { get; set; }
			public double divisionXpInfantry { get; set; }
			public double divisionXpArmored { get; set; }
			public double losses { get; set; }
			public double shotsLanded { get; set; }
			public double divisionXpAirborne { get; set; }
			public double avgSpeed { get; set; }
			public double avgKillDistance { get; set; }
			public double score { get; set; }
			public double totalXp { get; set; }
			public double timePlayed { get; set; }
			public double matchesPlayed { get; set; }
			public double headshots { get; set; }
			public double divisionXpNone { get; set; }
			public double assists { get; set; }
			public double scorePerMinute { get; set; }
			public double shotsFired { get; set; }
			public double deaths { get; set; }
		}

		public class Mode2
		{
			public Raid raid { get; set; }
		}

		public class Weekly
		{
			public All2 all { get; set; }
			public Mode2 mode { get; set; }
		}

		public class Mp
		{
			public Lifetime lifetime { get; set; }
			public Weekly weekly { get; set; }
			public double level { get; set; }
			public double maxLevel { get; set; }
			public double levelXpRemainder { get; set; }
			public double levelXpGained { get; set; }
			public double prestige { get; set; }
			public double prestigeId { get; set; }
			public double maxPrestige { get; set; }
		}

		public class All3
		{
		}

		public class Mode3
		{
		}

		public class Lifetime2
		{
			public All3 all { get; set; }
			public Mode3 mode { get; set; }
		}

		public class All4
		{
		}

		public class Mode4
		{
		}

		public class Weekly2
		{
			public All4 all { get; set; }
			public Mode4 mode { get; set; }
		}

		public class Zombies
		{
			public Lifetime2 lifetime { get; set; }
			public Weekly2 weekly { get; set; }
		}

		public class Data
		{
			public string title { get; set; }
			public string platform { get; set; }
			public string username { get; set; }
			public Mp mp { get; set; }
			public Zombies zombies { get; set; }
			public object engagement { get; set; }
		}

		public string status { get; set; }
		public Data data { get; set; }
	}
}
